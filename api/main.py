...
_possible_html_paths = [
    ...
    Path("/var/task") / "api" / "index.html",
    Path("/var/task") / "index.html",
    BASE_DIR / "index.html",
    Path.cwd() / "index.html",
    BASE_DIR / "templates" / "index.html",
    BASE_DIR / "api" / "dashboard.html",
    BASE_DIR / "static" / "index.html",
]

for _p in _possible_html_paths:
    logger.info(f"Checking path: {_p}")
    if _p.exists():
        try:
            ...
            logger.error(f"Error reading {_p}: {e}")

if _INDEX_HTML_CONTENT is None:
    # Final check: search recursively in BASE_DIR for index.html if still missing
    logger.warning(f"Searching recursively for index.html in {BASE_DIR}")
    for _f in BASE_DIR.rglob("index.html"):
        if not str(_f).endswith(".pyc"):
            _INDEX_HTML_CONTENT = _f.read_text(encoding="utf-8")
            logger.info(f"Loaded index.html recursively from: {_f}")
            break
...
async def root():
    ...
    # Fallback with debug info if template doesn't exist
    try:
        from pathlib import Path
        import os
        
        # Check current and base directories
        files_base = []
        if BASE_DIR.exists() and BASE_DIR.is_dir():
            files_base = [f"{f.name} ({'Dir' if f.is_dir() else 'File'})" for f in BASE_DIR.iterdir()]
        else:
            files_base = ["BASE_DIR_NOT_DIRECTORY"]
            
        files_cwd = [f"{f} ({'Dir' if os.path.isdir(f) else 'File'})" for f in os.listdir(".")]
        
        # Check /var/task explicitly (common Vercel root)
        files_var_task = []
        if os.path.exists("/var/task"):
            files_var_task = [f"{f} ({'Dir' if os.path.isdir(os.path.join('/var/task', f)) else 'File'})" for f in os.listdir("/var/task")]
        
        return HTMLResponse(content=f"""
        <html>
            <head>
                <title>DEBUG - Stock Intelligence Dashboard</title>
                <style>body {{ font-family: monospace; padding: 20px; }} code {{ background: #eee; }}</style>
            </head>
            <body>
                <h1>DEBUG: index.html not found</h1>
                <p><strong>BASE_DIR:</strong> <code>{BASE_DIR}</code></p>
                <p><strong>CWD:</strong> <code>{os.getcwd()}</code></p>
                
                <h3>Files in /var/task (Production Root):</h3>
                <ul>{"".join(f"<li>{f}</li>" for f in files_var_task)}</ul>
                
                <h3>Files in BASE_DIR:</h3>
                <ul>{"".join(f"<li>{f}</li>" for f in files_base)}</ul>
                
                <h3>Files in CWD:</h3>
                <ul>{"".join(f"<li>{f}</li>" for f in files_cwd)}</ul>
                
                <hr>
                <p>Welcome to the Stock Intelligence API. Visit <a href="/docs">/docs</a> for API documentation.</p>
            </body>
        </html>
        """)
    except Exception as e:
        return HTMLResponse(content=f"<h1>Debug Failure</h1><p>Error in debug listing: {str(e)}</p>")
